        ...
        <div class=" nobottommargin">
            <section data-v-5790ee72="" id="page-title" class="page-title-pattern bottommargin-sm">
                <div data-v-5790ee72="" class="container clearfix">
                    <h1 data-v-5790ee72="">团队订单</h1>
                    <span data-v-5790ee72="">Power of my Team</span>
                </div>
            </section>
            <div>
            ...
            <el-table
                ...
                <el-table-column type="expand">
                    <template slot-scope="props">
                        ...
                        <div class="col_half title-block col_last">
                            ...
                            <p style="margin:15px">
                                <span class="my_title">顧客:</span>{{props.row.customer.name}} <br>
                                <span class="my_title">邮编:</span>{{props.row.customer.postcode}} <br>
                                <span class="my_title">电话:</span>{{props.row.customer.phone}} <br>
                                <span class="my_title">地址1:</span>{{props.row.customer.address1}} <br>
                                <span class="my_title">地址2:</span>{{props.row.customer.address2}} <br>
                                <span class="my_title">邮箱:</span>{{props.row.customer.email}}
                            </p>
                        ...
                        <div class="col_one_fourth  title-block d-block">
                            ...
                            <a :href="'/po/print/open/'+props.row.id" class="btn btn-info text-danger d-block my-10">
                                <i class="icon-file-pdf"></i>打印客户订单
                            </a><a :href="'/po/print/dis/'+props.row.id" class="btn btn-info text-danger d-block my-10">
                ...
                <el-table-column
                    ...
                    <template slot-scope="props">
                        ...
                        (<i class="icon-user color" v-if="ME.id==props.row.user.id"></i>
                        {{props.row.user.name}})
                    </template>
                </el-table-column>
                <el-table-column
                        ...
                        prop="cart_Dis_Price">
                    <template slot-scope="props">
                        {{props.row.cart_Dis_Price|currency_rmb}}
                    </template>
                </el-table-column>
                <el-table-column
                    ...
                    <template slot-scope="props">
                        ...
                        <a href="javascript:void(0);" @click="checkEMS(props.row.delivery_no)"
                           class="button button-3d button-mini button-rounded button-green"
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            loadStatusPOS(){
                this.$store.dispatch("pos/get_customeradmin_ofStatus",this.select_type).then(
                    resolve=>{
                        this.tableData=resolve.pos;
                        this.total=resolve.pos.length;
            ...
            RemovePO(poid){
                Swal.fire({
                    ...
                    return axios.post('/customerAdmin/pos/delete',{poid:poid})
                        ...
                        .then(data => {
                            this.$store.commit("pos/set_remove_teampo",poid)
