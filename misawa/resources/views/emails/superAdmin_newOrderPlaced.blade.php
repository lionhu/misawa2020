                                        ...
                                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                            ...
                                            <tr>
                                                <td align="left" colspan="2" class="padding-copy"
                                                    ...
                                                    <p style="margin-top:0;text-indent: 2em;">老板，来生意啦！你的用户 {{$po->user->name}} (#{{$po->user->id}}) 给你下了一个新订单！</p>
                                    ...
                                    <tr>
                                        <td align="right" class="padding-copy articletextlightStyle"
                                            ...
                                            <p style="margin-top:0px">
                                                数量：{{$cartitem->qty}} <br>
                                                R_Price: <span class="money_jp">{{$cartitem->product->r_price}} </span> /
                ...
                <table border="0" cellpadding="0" cellspacing="0" width="500" style="padding: 0 0 20px 0;"
                    ...
                    <tr>
                        <td align="center" class="padding-copy" colspan="2"
                            style="padding: 0 0 10px 0; font-size: 25px; font-family: Helvetica, Arial, sans-serif; font-weight: normal; color: #ffffff;">
                            总计数量：{{$po->cart_Qty}} <br>
                            Vendor进价：<span class="money_jp">{{$po->cart_R_Price}} 円</span>/R_利润：<span class="money_jp">{{round($po->cart_B_Price*env("CURRENCY_RATE") - $po->cart_R_Price,-1)}}円</span><br>
                            B_卖价：<span class="money">{{$po->cart_B_Price}} 元</span><br>

                            @if($po->cart_Dis_Price-$po->cart_B_Price > 0)
                                Distributor_卖价：<span class="money">{{$po->cart_Dis_Price}} 元</span><br>
                                返回提成金额:<span class="money">{{$po->cart_Dis_Price-$po->cart_B_Price}} 元</span> <br>
                                渠道管理员：{!! $clientAdmin->name !!}(#{!! $clientAdmin->id !!})
