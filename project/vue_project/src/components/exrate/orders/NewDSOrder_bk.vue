...
<script>
  ...
  export default {
    ...
    computed:{
      // systemEnvs(){
      //   console.log(this.$store.state.system.systemEnvs.params)
      //   console.log(this.$store.state.system.systemEnvs.params.fixed_rate)
    ...
    mounted() {
      ...
      this.order.user= this.ME.user.id;
        ...
        console.log("store dispatch systemEnvs");
      this.$store.dispatch("system/get_systemEnvs").then(
        resolve=>{
          console.log(resolve)
             this.systemEnvs = resolve.params.fixed_rate;
            var rj=parseFloat(this.systemEnvs.offset_rj)+parseFloat(this.todayrate.hui_out)
            this.traderate_rj = rj.toFixed(4);
            var jr=parseFloat(this.systemEnvs.offset_jr)+parseFloat(this.todayrate.chao_in)
            this.traderate_jr = jr.toFixed(4);
        },reject=>{})
